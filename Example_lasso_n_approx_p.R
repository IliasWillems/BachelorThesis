...
for(k in 0:(Number_Of_iterations - 1)) {
  ...
  # Apply the Lasso on the data. Below we create a model based on 4
  # predictors for the variable 'Salary'. Note that the best lambda is auto-
  # matically selected by the function cv.glmnet()
  cv_fit = cv.glmnet(X, y, alpha = 1, lambda = lambdas, intercept = TRUE, standardize = FALSE)
...
for (i in 1:length(lambdas)) {
  ...
  # Store the regression coefficients in a vector
  betas[,i] = predict(lasso_model, type = "coefficients", s = lambda)[1:5]
...
plot(log(lambdas), MSE, "l", main = "A plot of the MSE (Lasso) in function of lambda")
...
plot(log(lambdas), betas[2,], "l", col = cl[2], ylim = c(-5,7), 
     ylab = "Coefficients", main = "A plot of the coefficients (Lasso) in function of lambda")
